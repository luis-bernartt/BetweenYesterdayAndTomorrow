...
screen quick_menu():
    ...
    if quick_menu:
        ...
        vbox:
            ...
            textbutton _("Q.Save") action ShowMenu('save')
            textbutton _("Q.Load") action ShowMenu('load')
            textbutton _("Mute"):
                action Function(mute_all_to_default)
                selected (
                        preferences.get_volume("music") <= 0.0
                        and preferences.get_volume("sfx") <= 0.0
                        and preferences.get_volume("voice") <= 0.0
                    ) 
...
style menu_frame:
    background Frame("gui/menu_bg.png", 20, 20)
    align (0.5, 0.6)
    padding (120, 40)
...
style all_bar:
    ...
    thumb "gui/slider/horizontal_custom_thumb.png"
    left_gutter  0
    right_gutter 5 
    thumb_offset (0, 10)
    xysize (500, 100)
    ...
    use game_menu(_("Audio"), scroll="viewport"):
        vbox:
            if config.has_music:
                ...
                hbox:
                    if preferences.get_volume("music") == 0:
                        add Transform("gui/audio_button_off.png", zoom=0.1)
                    else:
                        add Transform("gui/audio_button_on.png", zoom=0.1)
                    bar:
            ...
            if config.has_voice:
                ...
                hbox:
                    if preferences.get_volume("voice") == 0:
                        add Transform("gui/audio_button_off.png", zoom=0.1)
                    else:
                        add Transform("gui/audio_button_on.png", zoom=0.1)
                    bar:
...
init -10 python:
    def mute_all_to_default():
        if getattr(preferences, "all_mute", False):
            preferences.all_mute = False
            preferences.set_volume("music", 0.10)
            preferences.set_volume("sfx", 0.10)
            preferences.set_volume("voice", 0.10)
        else:
            preferences.set_volume("music", 0.0)
            preferences.set_volume("sfx", 0.0)
            preferences.set_volume("voice", 0.0)
            preferences.all_mute = True
